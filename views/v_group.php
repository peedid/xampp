...
<div data-role="page" id="page_utama" data-theme="a" data-transition="slide">
	<?php 
      ...
      <div data-role="content">
      	<div align="center" style="margin-top: -25px" ><h3><b>DAFTAR GROUP</b></h3></div>
      	<div>
             ...
             <td>
              <table align="center">
                <tr>
                  ...
                  <td>
                    <div align="center" style=" background-color: orange; width:30px;border-radius: 50%;height:30px;" onclick="click_edit('<?= $row->KODE_GROUPBARANG;?>')">
                      <img   id="tambah_data" src="<?php echo asset_url();?>gambar/ubah.png" style="width:20px;height:20px; margin-top: 5px;">
                    </div>
                  </td>
                  <td>
                    <div align="center" style=" background-color: red; width:30px;border-radius: 50%;height:30px;" onclick="click_hapus('<?= $row->KODE_GROUPBARANG;?>')">
...
<div data-role="page" id="page_data" data-transision="slide">
  <?php 
    ...
    <div data-role='content'>     
        <div align="center">
       <h3 id="tajuk"><b>TAMBAH DATA GROUP BARU</b></h3>
        </div>
        <div>
                              <?php echo form_open_multipart ("",array("method"=>"post","id"=>"form_gudang","name"=>"form_gudang","data-ajax"=>"false", "enctype"=>"multipart/form-data")) ?> 
                                ...
                                <label>Kode Group Barang:</label>
                                <input data-mini="true" type="text" name="kode_group_add" id="kode_group_add" value="">
...
<script type="text/javascript">
 ...
 function click_detil(kode) {
  $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
      ...
      document.getElementById("kode_group_add").value = kode;
      document.getElementById("tajuk").innerHTML = 'DETAIL DATA GROUP BARANG';
      document.getElementById("kode_group_add").disabled = true;
 ...
 function click_edit(kode) {
  $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
      ...
      document.getElementById("tajuk").innerHTML = 'UBAH DATA GROUP BARANG';
      document.getElementById("kode_group_add").disabled = false;
...
function click_hapus(id){ 
        ...
        document.getElementById('isi_hapus').innerHTML = 'Data Group Barang ' + id + ' akan di hapus ?';
...
$('#form_delete').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        showModal();
        $.ajax({
                url:'<?php echo home_url()?>MasterData/delete_group_barang',
                method: 'POST',
...
function add_data(){
   ...
   $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
      document.getElementById("kode_group_add").value = null;
      document.getElementById("key").value = null;
      document.getElementById("tajuk").innerHTML = 'TAMBAH DATA GROUP BARANG';
      document.getElementById("kode_group_add").disabled = false;
      status  = 'tambah';
      // location.reload()
}
...
$('#form_gudang').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        if (status == "tambah") {
              alamat = '<?php echo home_url()?>MasterData/save_group_barang'
        }
        else
        {
          alamat = '<?php echo home_url()?>MasterData/edit_group_barang'
        }
