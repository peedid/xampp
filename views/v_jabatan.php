...
<div data-role="page" id="page_utama" data-theme="a" data-transition="slide">
	<?php 
      ...
      <div data-role="content">
      	<div align="center" style="margin-top: -25px" ><h3><b>DAFTAR DATA JABATAN</b></h3></div>
      	<div>
             ...
             <td>
              <table align="center">
                <tr>
                  ...
                  <td>
                    <div align="center" style=" background-color: orange; width:30px;border-radius: 50%;height:30px;" onclick="click_edit('<?= $row->JABATAN;?>')">
                      <img   id="tambah_data" src="<?php echo asset_url();?>gambar/ubah.png" style="width:20px;height:20px; margin-top: 5px;">
                    </div>
                  </td>
                  <td>
                    <div align="center" style=" background-color: red; width:30px;border-radius: 50%;height:30px;" onclick="click_hapus('<?= $row->JABATAN;?>')">
...
<div data-role="page" id="page_data" data-transision="slide">
  <?php 
    ...
    <div data-role='content'>     
        <div align="center">
       <h3 id="tajuk"><b>TAMBAH DATA JABATAN BARU</b></h3>
        </div>
        <div>
                              <?php echo form_open_multipart ("",array("method"=>"post","id"=>"form_gudang","name"=>"form_gudang","data-ajax"=>"false", "enctype"=>"multipart/form-data")) ?> 
                                ...
                                <label>Jabatan:</label>
                                <input data-mini="true" type="text" name="jabatan_add" id="jabatan_add" value="">
...
<script type="text/javascript">
 ...
 function click_edit(kode) {
  $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
      document.getElementById("jabatan_add").value = kode;
      document.getElementById("key").value = kode;
      document.getElementById("tajuk").innerHTML = 'UBAH DATA JABATAN';
...
function click_hapus(id){ 
        ...
        document.getElementById('isi_hapus').innerHTML = 'Data Jabatan ' + id + ' akan di hapus ?';
...
$('#form_delete').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        showModal();
        $.ajax({
                url:'<?php echo home_url()?>MasterData/delete_jabatan',
                method: 'POST',
...
function add_data(){
$.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
      document.getElementById("jabatan_add").value = null;
      document.getElementById("key").value = null;
      document.getElementById("tajuk").innerHTML = 'TAMBAH DATA JABATAN';
      status  = 'tambah';
...
$('#form_gudang').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        if (status == "tambah") {
              alamat = '<?php echo home_url()?>MasterData/save_jabatan'
        }
        else
        {
          alamat = '<?php echo home_url()?>MasterData/edit_jabatan'
        }
