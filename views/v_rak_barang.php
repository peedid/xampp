...
<div data-role="page" id="page_utama" data-theme="a" data-transition="slide">
	<?php 
      ...
      <div data-role="content">
      	<div align="center" style="margin-top: -30px" ><h3><b>DATA RAK BARANG</b></h3></div>
      <div>
             ...
             <td>
               <table width="100%" style="height: 50px; " >
                 <tr style="text-align:center;background-color:#2C3E50;color:white;  " >
                  <td align="center" width="30%"  style=" border: 1px solid;border-top-left-radius: 5px; border-bottom-left-radius: 5px; "> 
                    Kode Rak Barang
                  </td>
                   <td align="center" width="45%"  style=" border: 1px solid;">
                   Rak Barang
                 </td>
             ...
             <div class="flex-container" style="width: 100%">
              ...
              <table style="width: 100%">
                <tr align="center">
                  ...
                  <td>
                    <div align="center" style="background-color: orange; width:30px;border-radius: 50%;height:30px;" onclick="click_edit('<?= $row->KODE_RAKBARANG; ?>','<?= $row->RAK_BARANG; ?>')">
                      <img   id="tambah_data" src="<?php echo asset_url();?>gambar/ubah.png" style="width:20px;height:20px; margin-top: 5px;">
                    </div>
                  </td>
                  <td>
                    <div align="center" style="background-color: red; width:30px;border-radius: 50%;height:30px;" onclick="click_hapus('<?= $row->KODE_RAKBARANG; ?>','<?= $row->RAK_BARANG; ?>')">
...
<div data-role="page" id="page_data" data-transision="slide">
  <?php 
    ...
    <div data-role='content'>
        <div align="center">
       <h3 id="tajuk"><b>TAMBAH DATA RAK BARANG BARU</b></h3>
        </div>
        <div>
                              <?php echo form_open_multipart ("",array("method"=>"post","id"=>"form_gudang","name"=>"form_gudang","data-ajax"=>"false", "enctype"=>"multipart/form-data")) ?> 
                                ...
                                <label id="kode_add_label" >Kode Rak Barang:</label>
                                <input data-mini="true" type="text" name="kode_add" id="kode_add" value="">
                                <label  >Rak Barang:</label>
                                <input data-mini="true" type="text" name="rak_barang_add" id="rak_barang_add" value="">
                                <?php echo form_close(); ?>
        </div>
...
function click_hapus(id, nama){ 
        ...
        document.getElementById('isi_hapus').innerHTML = 'Data Rak Barang ' + nama + ' akan di hapus ?';
...
$('#form_delete').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        showModal();
        $.ajax({
                url:'<?php echo home_url()?>MasterData/delete_rak_barang',
                method: 'POST',
...
function add_data(){
$.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
      document.getElementById("rak_barang_add").value = null;
      document.getElementById("key").value = null;
      document.getElementById("kode_add").value = null;
      document.getElementById("tajuk").innerHTML = 'TAMBAH DATA RAK BARANG';
      status  = 'tambah';
}
...
$('#form_gudang').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        if (status == "tambah") {
              alamat = '<?php echo home_url()?>MasterData/save_rak_barang'
        }
        else
        {
          alamat = '<?php echo home_url()?>MasterData/edit_rak_barang'
        }
