...
<div data-role="page" id="page_utama" data-theme="a" data-transition="slide">
	<?php 
      ...
      <div data-role="content">
      	<div align="center" style="margin-top: -30px" ><h3><b>DATA SALES</b></h3></div>
      <div>
             ...
             <div class="flex-container" style="width: 100%">
              ...
              <table style="width: 100%">
                <tr align="center">
                  ...
                  <td>
                    <div align="center" style="background-color: orange; width:30px;border-radius: 50%;height:30px;" onclick="click_edit('<?= $row->KODE_SALES; ?>','<?= $row->NAMA_SALES; ?>')">
                      <img   id="tambah_data" src="<?php echo asset_url();?>gambar/ubah.png" style="width:20px;height:20px; margin-top: 5px;">
                    </div>
                  </td>
                  <td>
                    <div align="center" style="background-color: red; width:30px;border-radius: 50%;height:30px;" onclick="click_hapus('<?= $row->KODE_SALES; ?>','<?= $row->NAMA_SALES; ?>')">
...
<div data-role="page" id="page_data" data-transision="slide">
  ...
  <div data-role='content'>       
        <div align="center">
       <h3 id="tajuk"><b>TAMBAH DATA SALES BARU</b></h3>
        </div>
        <div>
                              <?php echo form_open_multipart ("",array("method"=>"post","id"=>"form_gudang","name"=>"form_gudang","data-ajax"=>"false", "enctype"=>"multipart/form-data")) ?> 
                                ...
                                <label id="kode_add_label" >Kode sales:</label>
                                <input data-mini="true" type="text" name="kode_add" id="kode_add" value="">
                                <label  >Nama sales:</label>
                                <input data-mini="true" type="text" name="sales_add" id="sales_add" value="">
                                <?php echo form_close(); ?>
        </div>
...
<script type="text/javascript">
 ...
 function click_edit(kode,nama) {
  $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
      document.getElementById("sales_add").value = nama;
      document.getElementById("key").value = kode;
      document.getElementById("kode_add").value = kode;
      document.getElementById("tajuk").innerHTML = 'EDIT DATA SALES';
      console.log(nama)
      status = 'edit';
...
function click_hapus(id, nama){ 
        ...
        document.getElementById('isi_hapus').innerHTML = 'Data sales ' + nama + ' akan di hapus ?';
...
$('#form_delete').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        showModal();
        $.ajax({
                url:'<?php echo home_url()?>MasterData/delete_sales',
                method: 'POST',
...
function add_data(){
    ...
    $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
      document.getElementById("sales_add").value = null;
      document.getElementById("key").value = null;
      document.getElementById("kode_add").value = null;
      document.getElementById("tajuk").innerHTML = 'TAMBAH DATA SALES';
      status  = 'tambah';
...
$('#form_gudang').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        if (status == "tambah") {
              alamat = '<?php echo home_url()?>MasterData/save_sales'
        }
        else
        {
          alamat = '<?php echo home_url()?>MasterData/edit_sales'
        }
