...
<body>
	 <?php
            ...
            $ds_hasil = $this->db->query("
               SELECT * FROM MST_TINGKATAN_HARGA;
               ");
             ?>
<div data-role="page" id="page_utama" data-theme="a" data-transition="slide">
	<?php 
      ...
      <div data-role="content">
      	<div align="center" style="margin-top: -30px" ><h3><b>DATA TINGKATAN HARGA</b></h3></div>
      <div>
            ...
            <td>
             <div class="flex-container" style="width: 100%">
              ...
              <div style="width: 10%;text-align: right;">
                <?= $row->PERSEN_HARGAJUAL; ?>
              </div>
              <div style="width: 1%; text-align: left; margin-left: -10px">
                %
              </div>
              <div style="width: 25%">
              <table style="width: 100%">
                <tr align="center">
                  ...
                  <td>
                    <div align="center" style="background-color: orange; width:30px;border-radius: 50%;height:30px;" onclick="click_edit('<?= $row->TINGKATAN_HARGA; ?>','<?= $row->PERSEN_HARGAJUAL; ?>')">
                      <img   id="tambah_data" src="<?php echo asset_url();?>gambar/ubah.png" style="width:20px;height:20px; margin-top: 5px;">
                    </div>
                  </td>
                  <td>
                    <div align="center" style="background-color: red; width:30px;border-radius: 50%;height:30px;" onclick="click_hapus('<?= $row->TINGKATAN_HARGA; ?>','<?= $row->PERSEN_HARGAJUAL; ?>')">
...
<div data-role="page" id="page_data" data-transision="slide">
  <?php 
      ...
      ?>
        ...
        <div align="center">
       <h3 id="tajuk"><b>TAMBAH DATA TINGKATAN HARGA BARU</b></h3>
        </div>
        <div>
                              <?php echo form_open_multipart ("",array("method"=>"post","id"=>"form_gudang","name"=>"form_gudang","data-ajax"=>"false", "enctype"=>"multipart/form-data")) ?> 
                                ...
                                <label id="kode_add_label" >Kode Tingkatan Harga:</label>
                               ...
                               <table width="70%" style="margin-top: -10px">
                                 <tr>
                                   ...
                                   <td style="width: 45%"> 
                                    <div class="flex-container" style="width: 100%">
                                        <div style="width: 70%">
                                          <input data-mini="true" type="number" name="persen_add" id="persen_add" value="">
                                        </div>
                                        <div style="width: 3%; margin-left: -10px">
                                          <p>%</p>
                                        </div> 
                                    </div>
                                   </td>
...
<script type="text/javascript">
 ...
 function click_edit(kode,persen) {
  $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
      document.getElementById("persen_add").value = persen;
      document.getElementById("key").value = kode;
      document.getElementById("kode_add").value = kode;
      document.getElementById("tajuk").innerHTML = 'EDIT DATA TINGkATAN HARGA';
      console.log(persen)
      status = 'edit';
...
function click_hapus(id){ 
        ...
        document.getElementById('isi_hapus').innerHTML = 'Data Tingkatan Harga ' + id + ' akan di hapus ?';
...
$('#form_delete').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        showModal();
        $.ajax({
                url:'<?php echo home_url()?>MasterData/delete_tingkatan_harga',
                method: 'POST',
...
$('#form_gudang').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        if (status == "tambah") {
              alamat = '<?php echo home_url()?>MasterData/save_tingkatan_harga'
        }
        else
        {
          alamat = '<?php echo home_url()?>MasterData/edit_tingkatan_harga'
        }
