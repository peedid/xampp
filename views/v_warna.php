...
<div data-role="page" id="page_utama" data-theme="a" data-transition="slide">
  ...
  <div data-role="content">
    <div align="center" style="margin-top: -25px" ><h3><b>DAFTAR WARNA</b></h3></div>
    <div>
          ...
          <td>
            ...
            <td>
              <table align="center">
                <tr>
                  <td>
                    <div align="center" style=" background-color: orange; width:30px;border-radius: 50%;height:30px;" onclick="click_edit('<?= $row->WARNA;?>')">
                      <img   id="tambah_data" src="<?php echo asset_url();?>gambar/ubah.png" style="width:20px;height:20px; margin-top: 5px;">
                    </div>
                  </td>
                  <td>
                    <div align="center" style=" background-color: red; width:30px;border-radius: 50%;height:30px;" onclick="click_hapus('<?= $row->WARNA;?>')">
...
<div data-role="page" id="page_data" data-transision="slide">
  <div data-role="content" >
    ...
    <div align="center">
      <h3 id="tajuk"><b>TAMBAH DATA WARNA BARU</b></h3>
    </div>
    <div>
      ...
      <label>Warna:</label>
      <input data-mini="true" type="text" name="warna_add" id="warna_add" value="">
      <?php echo form_close(); ?>
    </div>
    <div align="center">
...
function click_detil(kode) {
  $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
    document.getElementById("warna_add").value = kode;
    document.getElementById("tajuk").innerHTML = 'DETAIL DATA WARNA';
...
function click_edit(kode) {
  $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
    ...
    document.getElementById("tajuk").innerHTML = 'UBAH DATA WARNA';
      status = 'edit';
}

function click_hapus(id){
  $('#btn_delete').attr('hapus_id',id );
    document.getElementById('isi_hapus').innerHTML = 'Data Warna ' + id + ' akan di hapus ?';
...
$('#form_delete').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        showModal();
        $.ajax({
                url:'<?php echo home_url()?>MasterData/delete_warna',
                method: 'POST',
...
function add_data(){
  ...
  $.mobile.changePage( "#page_data", { transition: "slide", changeHash: true });
    document.getElementById("warna_add").value = null;
    document.getElementById("key").value = null;
    document.getElementById("tajuk").innerHTML = 'TAMBAH DATA WARNA';
    status  = 'tambah';
...
$('#form_gudang').submit(function(event){
      ...
      if ($('#id').val()=="") {
        ...
        if (status == "tambah") {
              alamat = '<?php echo home_url()?>MasterData/save_warna'
        }
        else
        {
          alamat = '<?php echo home_url()?>MasterData/edit_warna'
        }
